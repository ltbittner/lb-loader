...
class LBLoader {
    constructor(args = {}) {
        ...
        this.sequential = args.sequential || false;
    ...
    startPreload() {
        ...
        if(!this.sequential) {
            ...
            this.loadAssetsSequential(this.preloadQueue, this.preloadProgressCallback, this.preloadCompletedCallback, 0);
        }
    ...
    startBackgroundLoad() {
        ...
        if(!this.sequential) {
            this.loadAssetsSequential(this.preloadQueue, this.preloadProgressCallback, this.preloadCompletedCallback);
        } else {
            this.loadAssetsSequential(this.backgroundLoadQueue, this.backgroundLoadProgressCallback, this.backgroundLoadCompletedCallback, 0);
        }
    }
    ...
    loadAssetsSequential(queue, progressCallback, completeCallback, index) {
        let asset = queue[index];

        this.loadAsset(asset.src, asset.type).then(() => {
            this.progressHandler(queue, index, progressCallback);
            if(index+1 < queue.length)
                this.loadAssetsSequential(queue, progressCallback, completeCallback, index+1);
            else
                completeCallback();
        }).catch(() => {
            console.error(new Error('Error in loading \'' + asset.src + '\'.'));
